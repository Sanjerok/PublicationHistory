...
def get_layout():
    ...
    Add_New_Publication = html.Div([
        ...
        dbc.Toast(
            ...
            id="pub-error-popup",  # Changed from status-error-popup-text
            header="Error!",
            is_open=False,
            dismissable=True,
            duration=5000,
...
@callback(
    ...
    Output("pub-error-popup", "is_open"),
    Output("pub-error-popup", "children"),
...
def add_new_publication(n_clicks, Name, Ticket, Platform, Assignee, Start_Date, Feature_Flag, Planned_Sites, Planned_Devices, Planned_Genders, Planned_Languages):
    ...
    Required_fields = {"Name": Name, "Ticket": Ticket, "Platform": Platform,  "Planned_Sites": Planned_Sites, "Planned_Devices": Planned_Devices, "Planned_Genders": Planned_Genders, "Planned_Languages": Planned_Languages}
    is_null = [i for i in Required_fields.keys() if Required_fields[i] is None or Required_fields[i] == ""]
    
    if n_clicks and not Name_check and not is_null:
        if Name and Ticket:
            ...
            return True, False, "input-field", "input-field", False, ""
        check_name = "input-field" if Name else "input-field-error"
        check_link = "input-field" if Ticket else "input-field-error"
        return False, False, check_name, check_link, False, ""
    elif n_clicks and Name_check:
        return False, True, "input-field-error", "input-field", False, ""
    elif n_clicks and is_null:
        return False, False, "input-field", "input-field", True, f"Please fill all required field: {', '.join(is_null)}"
    else:
        return False, False, "input-field", "input-field", False, ""
    # fix this
...
def search(platform, assignees, date_start, date_end, n_clicks, publication, status, _):
    ...
    publication_table = publications_df[
                                        ...
                                        & 
                                        (publications_df.Name.isin(publication) if publication is not None and publication != "" and publication != [] else true)
                                        & 
                                        (publications_df['Current Status'].isin(status) if status is not None and status != [] else true)
    ]
    publication_table_publications = publications_df[
                                        (publications_df.Assignee.isin(assignees) if assignees is not None and assignees != [] else true)
                                        &
                                        (pd.to_datetime(publications_df['Start Date']).isin(pd.date_range(date_start,date_end)) if date_start is not None and date_end is not None else true)
                                        & 
    ...
    publications_options = list(set(publication_table_publications.fillna("").Name.tolist()))
...
@callback(
    ...
    Output('pub-error-popup', 'is_open', allow_duplicate=True),
    Output('pub-error-popup', 'children', allow_duplicate=True),
...
def update_table(data, n_clicks):
    ...
    if n_clicks > 0:
        ...
        df, is_error_open, is_error_msg = dm.update_publications(data)
        n_clicks = 0
        return n_clicks, df.to_dict("records"), not is_error_open, is_error_open, is_error_msg
    return n_clicks, data, False, False, ""
...
def dropdowns(data, data_previous):
    ...
    if data_previous and df.shape == df1.shape:
        ...
        if len(diff_indices) >= 0:
            ...
            print(f"The DataFrames are identical. - {len(diff_indices)}")
    ...
    dropdown =     {
                 "Platform": {"options": [{'label': i, 'value':i} for i in sorted(dd.PLATFORM_OPTIONS)]},
                 "Assignee": {"options": [{'label': i, 'value':i} for i in sorted(dd.ASSIGNEE_OPTIONS)]},
                 "Planned Genders": {"options": [{'label': i, 'value':i} for i in sorted(dd.GENDER_OPTIONS)]},
    ...
    dropdown_conditional = [
...

        {
            ...
            "if": {
                        'column_id': 'Planned Sites',
                        'filter_query': '{Platform} eq "Retention"'
                },
                
               "options": [{'label': i, 'value':i} for i in site_options if i != 'StagingCupid']
        },
    ...
    link_is_open = False
    if data_previous:
        data, date_is_open, date_error_msg = dm.date_validation(data, data_previous, ['Start Date'])
